...
export const FeaturedProjects: React.FC = () => {
  ...
  const flagshipProject = FEATURED_PROJECTS.find((p) => p.id === 'ghcookwriter') || FEATURED_PROJECTS[0]; // GHCOOKwriter Author Site
  
  // Explicitly curated homepage secondary featured items (#2 Relocatia, #3 Signs & Banners, #4 Heritage Soul Kitchen)
  const featuredIds = ['relocatia', 'signs-banners-branding', 'hsk'];
  const otherProjects = featuredIds
    .map((id) => FEATURED_PROJECTS.find((p) => p.id === id))
    .filter((p): p is typeof FEATURED_PROJECTS[0] => Boolean(p));
